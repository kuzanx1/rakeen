-- ══════════════════════════════════════════════════════════════════
--   شغّل هذا الملف كاملاً، مرّةً واحدة. لا يهمّ ما شُغّل قبله:
--   كلُّ جزءٍ فيه idempotent -- إعادتُه لا تضرّ ولا تكرّر.
--
--   ١) رمز تأكيد المكافأة  (إنشاء + تحقّق)
--   ٢) محو الرمز بعد استعماله + التأكيد بمسح البطاقة
--   ٣) إعادة الرمز الحيّ بدل توليد جديد
--   ٤) التأكيد برقم البطاقة (الأحرف الثمانية)
--   ٥) عمود is_free_reward + complete_pos_order تكتبه
--   ٦) طريقة دفع 'loyalty'          ← بدونها يفشل حفظ طلبٍ مجانيّ كامل
--   ٧) الاسترجاع سطراً سطراً + إشعاره
--   ٨) سلامة الولاء: المجاني لا يُعدّ + سحب ما رُجّع
--   ٩) إعادة مكافأةٍ خُصمت ولم تُصرف
-- ══════════════════════════════════════════════════════════════════


-- ═════ 20260907030000_loyalty_redemption_code ═════
...
-- ═════ 20260907040000_loyalty_code_cleanup_and_card ═════
...
-- ═════ 20260907050000_loyalty_code_reuse ═════
...
-- ═════ 20260907060000_loyalty_confirm_by_number ═════
...
-- ═════ 20260907070000_order_item_free_reward ═════
...
-- ═════ 20260907080000_payment_method_loyalty ═════
...
-- ═════ 20260907090000_partial_refunds ═════
...
-- ═════ 20260907100000_loyalty_integrity ═════
...
-- ═════ 20260907110000_return_unused_reward ═════
