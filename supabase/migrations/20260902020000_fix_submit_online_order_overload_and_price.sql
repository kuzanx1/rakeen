-- Corrects a real mistake in 20260902010000_submit_online_order_price_override.sql:
-- that migration redefined submit_online_order using a STALE 13-parameter
-- signature copied from an older migration, missing the 14-parameter
-- version actually live in production (20260831110000_pickup_delivery_stage_tracking.sql,
-- which added p_scheduled_by_customer). Because the signatures differ,
-- `create or replace` created a SECOND overloaded function instead of
-- replacing the real one — breaking every online-order submission with a
-- PostgREST "could not choose the best candidate function" error, and
-- silently reverting the is_active business check + the qty range
-- validation that 20260829190000/20260831110000 had already added.
--
-- This drops the wrong-signature overload and re-establishes the correct,
-- full 14-parameter function — identical to 20260831110000's version, with
-- only the same targeted online_price fix (a line's price now prefers
-- menu_items.online_price when set, falling back to price otherwise).

drop function if exists public.submit_online_order(text, text, text, text, text, text, jsonb, bigint, numeric, numeric, timestamptz, uuid, text);
...
begin
  ...
  loop
    ...
    insert into order_items (order_id, menu_item_id, qty, unit_price, modifiers_total, line_total, selected_modifiers, note, cost_at_sale)
      values (
        v_order_id, v_menu_item.id, v_qty, v_base_price, v_line_mods,
        v_line_price * v_qty,
