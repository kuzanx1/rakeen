-- الهدر يُطرح من صافي الربح.
--
-- في 20260916070000 عُرض الهدر بندًا مستقلًّا ولم يُطرح -- عمدًا، لئلّا
-- يتغيّر رقمٌ يقرأه أصحاب المطاعم منذ شهور بلا أن يطلبوا. وقد طُلب.
--
-- ▓ لماذا لا ازدواج:
--   تكلفة البضاعة المباعة تُحسب من أصناف الطلبات المباعة وحدها
--   (order_items)، والهدر لا يمرّ ببيع قطّ فلا سطر له هناك. فالمادتان
--   منفصلتان تمامًا، ومجموعهما لا يعدّ غرامًا مرّتين.
--
-- ▓ ولماذا بندٌ مستقلّ لا إضافةٌ إلى COGS:
--   تكلفة البضاعة تبقى «التكلفة النظرية لما بِيع» فتُقارن يومًا بيوم،
--   والفرق بينها وبين الاستهلاك الفعلي هو الهدر نفسه -- ودمجُهما يخفي
--   ذلك الفرق الذي يُقاس به الانضباط.
--
-- ▓ والتقارير المحفوظة قبل اليوم لا تتغيّر: هي لقطاتٌ مجمّدة، وصافي
--   ربحها مكتوبٌ فيها. التطبيق يبدأ من أوّل تقريرٍ يُولَّد بعد هذا.
--
-- لا يتغيّر التوقيع، فلا drop ولا إعادة revoke -- الصلاحيات تتبع
-- التوقيع وهو كما هو.
...
begin
  ...
  loop
    ...
    -- الهدر خسارةٌ حقيقية خرجت من الرفّ، فتُطرح. ولا تُضاف إلى
    -- v_cogs عمدًا -- انظر ترويسة هذا الملف.
    v_net_profit := v_gross_profit - coalesce(v_waste_cost, 0) - v_opex;
