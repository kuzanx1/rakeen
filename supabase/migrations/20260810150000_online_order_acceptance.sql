-- Online orders now land as 'pending' and require an explicit cashier
-- Accept/Reject on the POS (a blocking popup) before they become a real,
-- kitchen-visible 'completed' order. Rejecting requires a reason, which the
-- guest tracking page (get_order_status) surfaces back to the customer.
-- Purely additive to the status vocabulary — nothing existing is renamed.

alter table orders drop constraint orders_status_check;
alter table orders add constraint orders_status_check
  check (status in ('pending', 'completed', 'cancelled', 'refunded', 'rejected'));

alter table orders add column rejection_reason text;

-- ===== submit_online_order: insert as 'pending', not 'completed' =====
-- Body is otherwise byte-identical to the 20260810120000 version.
...
-- ===== accept_online_order — cashier accepts a pending online order =====
-- Mirrors mark_order_ready's shape. Unlike mark_order_ready's silent no-op
-- on zero rows, this raises so the POS can tell the cashier "another device
-- already handled this" instead of pretending the accept succeeded.
...
-- ===== reject_online_order — cashier rejects with a required reason =====
create or replace function reject_online_order(p_order_id bigint, p_reason text)
returns table(id bigint, status text, rejection_reason text)
language plpgsql
security definer
set search_path = public
as $$
...
begin
  ...
  where orders.id = p_order_id
    and orders.business_id = v_business_id
    and orders.status = 'pending'
  returning orders.id, orders.status, orders.rejection_reason;

  if not found then
    raise exception 'الطلب غير متاح للرفض — يمكن تم التعامل معه من جهاز آخر';
  end if;
end;
$$;

-- ===== get_order_status: add rejection_reason =====
