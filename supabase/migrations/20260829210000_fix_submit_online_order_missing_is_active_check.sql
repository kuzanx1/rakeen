-- Regression fix, layered on top of 20260829190000_fix_online_order_qty_validation.sql
-- (the latest redefinition of submit_online_order on disk at the time of
-- writing — confirmed via pg_get_functiondef against the live database that
-- this is also what's actually deployed today, modulo the qty-validation
-- migration itself which had not been applied yet).
--
-- submit_online_order lost its "and is_active = true" guard somewhere around
-- 20260821090000_geidea_payment_gateway.sql (which redefined the function
-- starting from the 20260819081000 body, dropping the is_active filter that
-- 20260814040000_admin_console_controls.sql had originally added). Every
-- subsequent redefinition of this function — including the qty-validation
-- fix above — carried that gap forward unnoticed, since none of them had a
-- reason to touch the business lookup line.
--
-- Net effect in production: suspending a business from /admin
-- (is_active = false) correctly locks its dashboard/POS/kitchen team out
-- (current_business_id()), and the storefront's own boot() JS refuses to
-- render the menu for it — but a caller who already knows the business's
-- slug could still invoke this anon-granted RPC directly and have it create
-- a real, chargeable order for a suspended business, bypassing the entire
-- point of "suspend". Restoring the is_active check here closes that gap.
--
-- Everything else in the function body is byte-identical to
-- 20260829190000_fix_online_online_order_qty_validation.sql — only the
-- business lookup's WHERE clause changes.
