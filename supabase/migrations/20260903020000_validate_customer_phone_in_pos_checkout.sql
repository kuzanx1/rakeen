-- Real reported bug: complete_pos_order()'s find-or-create-by-phone path
-- (20260901020000_restore_complete_pos_order_customer_find_or_create.sql)
-- only ever checked "non-empty" on p_customer_phone before inserting into
-- customers -- a customer got saved with an 11-digit phone number. The
-- POS's own #newCustPhoneInput field is now fixed client-side (maxlength=10
-- + digit-strip-on-input + /^05\d{8}$/ check before the "متابعة" button
-- enables), matching the pattern already used by the online-order
-- checkout's #omPhone field -- but this RPC is the real trust boundary
-- (callable directly with any payload, not just from this one client), so
-- client-side validation alone doesn't close the hole.
--
-- On an invalid/malformed phone: skip customer attachment entirely rather
-- than reject the whole sale -- a typo in an optional field shouldn't block
-- a real transaction. The order itself still completes and still stores
-- whatever was typed as free text (orders.customer_name/customer_phone),
-- it just won't create or match a customers row from a phone that isn't a
-- real 10-digit Saudi mobile number.
