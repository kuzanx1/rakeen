-- submit_online_order priced every line strictly off menu_items.price,
-- ignoring the new menu_items.online_price override — meaning a merchant
-- setting a different online price would see it reflected on the
-- storefront (client-side display only) while the actual server-trusted
-- charge silently used the wrong number. This is the ONLY change from the
-- function's previous definition (20260829140000_direct_cost_size_variant.sql):
-- the per-line price now prefers online_price when set, falling back to the
-- normal price otherwise. Everything else — validation, rate limiting,
-- scheduling, box/modifier handling, VAT — is reproduced verbatim.
