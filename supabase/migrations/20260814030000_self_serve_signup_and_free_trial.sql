-- Self-serve restaurant signup: a new business now gets online ordering
-- turned on and a default slug automatically (previously only ever set up
-- by hand via admin scripts during manual onboarding), so a self-registered
-- owner can open their storefront immediately with zero Rakeen involvement.
-- The slug defaults to a safe, always-unique `store-{id}` — the owner can
-- pick a nicer one later from Settings, this is just a working starting
-- point (Arabic business names don't transliterate reliably enough to
-- generate a nice slug automatically).
...
-- Free-trial tracking: the first 350 real online orders are free; after
-- that the storefront gates shut unless the owner has been marked
-- subscribed (no live payment gateway yet — v1 is "contact Rakeen, we flip
-- this after payment is arranged manually", see online_subscribed below).
alter table businesses add column online_order_free_count int not null default 0;
alter table businesses add column online_subscribed boolean not null default false;

-- The storefront needs to read these two columns to show the right
-- trial-ended state — extend anon's column-scoped grant (20260814010000)
-- to include them; still no other column is exposed.
grant select (online_order_free_count, online_subscribed) on businesses to anon;
