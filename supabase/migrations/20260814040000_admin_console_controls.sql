-- Platform-admin console, round 2: real operational control over a business
-- from /admin with zero hand-run SQL afterward — suspend/reactivate the
-- whole business (not just the online storefront), private admin notes,
-- a manually-set "paid until" date (no payment gateway yet, so this is set
-- by hand after an off-platform payment), and a per-business free-order
-- limit (was a hardcoded 350 in three places, now editable per restaurant
-- so admin can grant a bonus without touching the database).
...
alter table businesses add column subscription_expires_at timestamptz;
alter table businesses add column online_order_free_limit int not null default 350;

-- current_business_id() already returns null for a deactivated profile
-- (20260801230914), which transparently locks that one person out of every
-- RLS-protected table since every policy keys off business_id =
-- current_business_id(). Extending the same choke point to also require the
-- business itself be active means suspending a business from /admin locks
-- out its entire team (dashboard + POS + kitchen) in one write, reusing the
-- exact mechanism/error path already proven for the single-employee case —
-- no per-table policy changes, no new error handling needed anywhere.
...
as $$
  select p.business_id from profiles p
    ...
    where p.id = auth.uid() and p.active and b.is_active;
$$;

-- Storefront + free-order gate both need to read is_active/online_order_free_limit.
grant select (is_active, online_order_free_limit) on businesses to anon;
...
begin
  ...
  if not v_business.online_subscribed and v_business.online_order_free_count >= v_business.online_order_free_limit then
