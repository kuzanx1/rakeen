-- Real production regression found during the Feature Parity Pass
-- (React Native migration, Customer Management item) while verifying
-- customer attachment against the live backend.
--
-- complete_pos_order() originally had real find-or-create-by-phone
-- customer logic (see 20260802000753_customers_and_loyalty.sql, lines
-- 68-78, quoted verbatim below): typing a name+phone at checkout with
-- no known customer id created a real, searchable `customers` row.
--
-- 20260829200000_fix_pos_checkout_points_and_customer_id.sql rewrote
-- the function to accept a caller-supplied p_customer_id (so a client
-- that already resolved a real customer can pass its id directly) but
-- never re-added the find-or-create fallback for the case where the
-- caller has a name+phone but no id yet -- exactly the "type a new
-- customer at checkout" flow both the PWA and the React Native port
-- present to cashiers. Confirmed empirically against production: an
-- order created with only p_customer_name/p_customer_phone (no
-- p_customer_id) stores them as plain text on the order row and
-- creates NO customers row at all -- that customer becomes permanently
-- unsearchable/unreusable/ineligible for loyalty the moment the modal
-- closes, even though the UI in both clients implies a real customer
-- was just added.
--
-- Fix: restore the original find-or-create-by-phone block, now layered
-- UNDER the caller-supplied p_customer_id (a client that already knows
-- a real id skips this entirely; only a null id with a real phone
-- triggers find-or-create) -- the same precedence
-- register_dine_in_order/pay_dine_in_order already give an
-- explicitly-passed id. register_dine_in_order is NOT touched by this
-- migration -- confirmed it never had find-or-create logic at any point
-- in its history (it only ever stored customer_name/customer_phone as
-- free text), so this is restoring a real regression specific to
-- complete_pos_order, not extending capability to a function that never
-- had it.
