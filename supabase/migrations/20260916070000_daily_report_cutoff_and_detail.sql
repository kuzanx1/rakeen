-- التقرير اليومي: وقتُ إغلاقه يحدّده صاحب المكان، ومحتواه يلحق تحديثاتنا.
--
-- العطل الأول: يوم التقرير كان مثبّتًا من منتصف ليلٍ إلى منتصف ليل بتوقيت
-- الرياض. ومقهى يقفل الثانية فجرًا تقع مبيعاته بين ١٢ و٢ في تقرير الغد،
-- فيقرأ ليلته مقصوصةً نصفين ولا يطابق ما عدّه في الدرج. ومن يقفل الحادية
-- عشرة ليلًا كان سليمًا بالصدفة لا بالتصميم.
--
-- فصار لكل منشأة ساعةُ إغلاقٍ خاصة: يومُ العمل يبدأ عندها وينتهي عندها
-- بعد ٢٤ ساعة، والتقرير يُولَّد لحظةَ انتهائه لا في وقتٍ موحّد للجميع.
-- والمنادي (app/api/cron/daily-report) صار يعمل كل ساعة ويمرّر الساعة
-- الحالية بتوقيت الرياض، فتُعالَج المنشآت التي انتهى يومها في هذه الساعة
-- وحدها. لا خانة cron إضافية -- الحساب على الخطة المجانية سقفه خمس
-- خانات ونحن عندها، فالخانة اليومية القديمة صارت ساعيّة بدل أن تُزاد.
--
-- والعطل الثاني: التقرير ما كان يعرف شيئًا عمّا بنيناه بعده -- لا خصمًا
-- مفصّلًا بنسبه، ولا استرجاعًا، ولا هدرًا. فأُضيفت الثلاثة.
--
-- ملاحظة مقصودة: الهدر يُعرض بندًا مستقلًّا ولا يُطرح من صافي الربح.
-- طرحُه أدقّ محاسبيًّا، لكنّه يغيّر رقمًا يقرأه أصحاب المطاعم منذ شهور
-- بلا أن يطلبوا -- فيُعرض الرقم ويُترك القرار.
--
-- والتوقيع يتغيّر (بارامتر جديد)، فيلزم drop صريح قبل create or replace
-- وإلا PGRST203، ويلزم إعادة revoke لأن الصلاحيات تتبع التوقيع لا الاسم
-- (راجع 20260829190000_lock_down_internal_only_rpcs.sql).
...
do $chk$
begin
  if not exists (select 1 from pg_constraint where conname = 'businesses_day_cutoff_hour_ck') then
    alter table businesses add constraint businesses_day_cutoff_hour_ck
      check (day_cutoff_hour >= 0 and day_cutoff_hour <= 11);
  end if;
end
$chk$;

comment on column businesses.day_cutoff_hour is
  'ساعة انتهاء يوم العمل بتوقيت الرياض (٠ = منتصف الليل، ٢ = الثانية فجرًا). يوم التقرير = [اليوم عند هذه الساعة، الغد عند هذه الساعة).';

drop function if exists generate_daily_reports(date, timestamptz, timestamptz);
drop function if exists generate_daily_reports(date, timestamptz, timestamptz, smallint);

create or replace function generate_daily_reports(
  p_report_date date,
  p_day_start   timestamptz,
  p_day_end     timestamptz,
  p_cutoff_hour smallint default null
)
returns integer
language plpgsql
security definer
...
declare
  ...
  v_discount_breakdown jsonb; v_discount_orders integer;
  v_refunds_total numeric; v_refunds_count integer; v_refund_rows jsonb;
  v_waste_cost numeric; v_waste_rows jsonb;
  v_data jsonb;
begin
  for biz in
    select id, daily_report_sales, daily_report_products, daily_report_payments,
           daily_report_financial, daily_report_tax, daily_report_delivery
      from businesses
     where p_cutoff_hour is null or coalesce(day_cutoff_hour, 0) = p_cutoff_hour
  loop
    ...
    -- الخصم مفصّلًا بنسبه: «خصم ٥٠٪ -- ١٠ طلبات» بدل رقمٍ واحدٍ مجموع.
    -- نفس تفصيل الموازنة عند إقفال الوردية، حتى يُقرأ الاثنان معًا.
    select coalesce(jsonb_agg(jsonb_build_object('pct', pct, 'count', cnt, 'amount', amt) order by pct desc), '[]'::jsonb),
           coalesce(sum(cnt), 0)
      into v_discount_breakdown, v_discount_orders
      from (
        select round(coalesce(discount_pct,0))::int as pct, count(*) as cnt, coalesce(sum(coalesce(discount_amount,0)),0) as amt
        from tmp_day_orders where coalesce(discount_pct,0) > 0 group by 1
      ) db;

    -- الاسترجاع: يُحسب بوقت وقوعه لا بوقت الطلب الأصلي، فاسترجاعُ اليوم
    -- لطلب أمس يظهر في يوم الاسترجاع -- وهو اليوم الذي خرج فيه المال.
    select coalesce(sum(amount),0), count(*)
      into v_refunds_total, v_refunds_count
      from order_refunds
     where business_id = biz.id and created_at >= p_day_start and created_at < p_day_end;

    select coalesce(jsonb_agg(jsonb_build_object('orderId', order_id, 'amount', amount, 'kind', kind, 'reason', reason) order by created_at), '[]'::jsonb)
      into v_refund_rows
      from order_refunds
     where business_id = biz.id and created_at >= p_day_start and created_at < p_day_end;

    -- الهدر: من سجلّ الحركات نفسه (reason = 'waste')، بكلفته لا بكمّيته
    -- وحدها، فعشرون غرامًا من بنٍّ غالٍ ليست كعشرين من سكّر.
    select coalesce(sum((-sm.delta) * coalesce(si.unit_cost,0)), 0)
      into v_waste_cost
      from stock_movements sm join stock_items si on si.id = sm.stock_item_id
     where sm.business_id = biz.id and sm.reason = 'waste'
       and sm.created_at >= p_day_start and sm.created_at < p_day_end;

    select coalesce(jsonb_agg(jsonb_build_object('name', iname, 'qty', tqty, 'unit', iunit, 'cost', tcost, 'reason', top_reason) order by tcost desc), '[]'::jsonb)
      into v_waste_rows
      from (
        select si.name as iname, si.unit as iunit,
               sum(-sm.delta) as tqty,
               sum((-sm.delta) * coalesce(si.unit_cost,0)) as tcost,
               (array_agg(sm.note order by (-sm.delta) desc))[1] as top_reason
          from stock_movements sm join stock_items si on si.id = sm.stock_item_id
         where sm.business_id = biz.id and sm.reason = 'waste'
           and sm.created_at >= p_day_start and sm.created_at < p_day_end
    ...
    v_data := jsonb_build_object(
      ...
      'discountBreakdown', v_discount_breakdown, 'discountOrders', v_discount_orders,
      'refundsTotal', v_refunds_total, 'refundsCount', v_refunds_count, 'refundRows', v_refund_rows,
      'wasteCost', v_waste_cost, 'wasteRows', v_waste_rows,
      'dayStart', p_day_start, 'dayEnd', p_day_end, 'cutoffHour', coalesce(p_cutoff_hour, 0),
...
-- الصلاحيات تتبع التوقيع، والتوقيع تغيّر -- فتُعاد كما في
-- 20260829190000_lock_down_internal_only_rpcs.sql: لا يناديها إلا
-- service_role من المنادي.
revoke execute on function generate_daily_reports(date, timestamptz, timestamptz, smallint)
