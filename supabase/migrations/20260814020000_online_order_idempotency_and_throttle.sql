-- Two guest-checkout hardening fixes, ahead of onboarding many restaurants
-- at once:
--
-- 1) Idempotency: submit_online_order always did `gen_random_uuid()` for
--    client_order_uuid itself — a network retry or a double-tap before the
--    client's `btn.disabled = true` takes effect could create two real
--    orders for one customer intent, with nothing to detect it. Mirrors
--    complete_pos_order's existing p_client_order_uuid replay guard: the
--    client now generates the uuid once per checkout attempt and reuses it
--    across retries, so a repeat call returns the original order instead of
--    inserting a second one.
--
-- 2) Basic anti-spam: nothing anywhere blocks a script from calling this
--    anon-granted RPC in a loop to flood a restaurant's kitchen display
--    with fake orders. This is not a substitute for a Cloudflare-level
--    rate-limit rule (which the owner should still add), but it stops the
--    simplest case for free: the same phone number placing more than one
--    *new* order for the same business within a short window.
...
begin
  ...
  -- Idempotent replay — must come before any validation below, so a retry
  -- of an already-placed order always succeeds even if e.g. the cart or
  -- schedule would otherwise fail re-validation on a second look.
