-- Pickup scheduling + guest order tracking. Every online order today is
-- "ASAP, no visibility after submit" — this adds an optional customer-chosen
-- pickup time (server-validated against branch hours + prep-time setting)
-- and a random per-order tracking token so a guest can check status later
-- with zero login, mirroring the loyalty-card token pattern already used
...
drop function if exists submit_online_order(text, text, text, text, text, text, jsonb, bigint, numeric, numeric);
...
begin
  ...
  insert into orders (
    ...
    v_business.id, v_branch.id, null, p_channel, 'online', 'completed', 'cash', 0,
    0, 0, 0, 0, 0, 0,
...
-- Guest order-status lookup — same posture as get_loyalty_card: the random
-- token is the sole gate, resolved server-side, no auth session required.
...
returns table(
  ...
  business_name text, business_logo_url text, theme_color text
)
language plpgsql
security definer
set search_path = public
...
begin
  return query
    select o.id, o.channel, o.status, o.ready_at, o.scheduled_for, o.created_at, o.total, o.customer_name,
           b.name, b.logo_url, b.online_theme_color
