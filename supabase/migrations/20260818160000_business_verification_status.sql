-- Self-serve signup (20260814030000) creates a fully live storefront with
-- zero Rakeen review — anyone can register and start taking real customer
-- orders immediately. This adds the missing review gate: new self-serve
-- signups start 'pending' and can't take real online orders until a
-- platform admin verifies them. Existing businesses (everything already
-- live today, including manually-onboarded ones) default to 'verified' so
-- nothing already operating gets locked out by this migration.
alter table businesses add column verification_status text not null default 'verified'
  check (verification_status in ('pending', 'verified', 'rejected'));

-- New self-serve businesses start pending, not verified. Dashboard login and
-- setup (menu, branding, etc.) still work immediately — only real customer
-- orders on the storefront are gated below — so the owner can get their
-- menu ready while waiting on review instead of sitting idle.
...
begin
  if target_business_id is null then
    insert into businesses (name, verification_status)
    values (coalesce(meta->>'business_name', 'مشروعي'), 'pending')
...
-- The storefront needs to read this to show the right "pending review"
-- state — extend anon's column-scoped grant, same pattern as the free-trial
-- columns added alongside it.
grant select (verification_status) on businesses to anon;
