...
-- Guards against the same "cannot change return type" failure the loyalty
-- card fix hit (this function gets redefined again twice more later in this
-- same migration batch, and its live return columns may not match any one
-- file exactly) -- drop-then-create by argument types is safe regardless of
-- whether the live signature actually drifted.
drop function if exists public.submit_online_order(text, text, text, text, text, text, jsonb, bigint, numeric, numeric, timestamp with time zone, uuid, text);
