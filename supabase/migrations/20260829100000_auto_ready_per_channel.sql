-- Lets an owner skip the manual "جاهز" step per channel — some kitchens
-- genuinely don't need a prep-tracking handoff for every channel (e.g. a
-- grab-and-go counter where dine-in orders are always instantly ready).
-- Delivery is split into platform vs online because they already have
-- different endpoints (see 20260829090000_platform_orders_skip_delivered_stage.sql).
alter table businesses add column auto_ready_dine_in boolean not null default false;
alter table businesses add column auto_ready_pickup boolean not null default false;
alter table businesses add column auto_ready_delivery_platform boolean not null default false;
alter table businesses add column auto_ready_delivery_online boolean not null default false;

-- complete_pos_order: cashier-entered orders (dine_in / pickup / delivery-
-- via-platform, since only submit_online_order ever writes source='online').
-- A delivery order auto-marked ready this way is, by the same reasoning as
-- the platform-orders migration above, also auto-marked delivered — it's
-- never an online order at this call site.
...
-- register_dine_in_order: only the fresh-order branch (not the "add items to
-- an existing unpaid table order" merge branch — a ticket already underway
-- shouldn't silently flip back to "ready" just because more items were added).
...
-- accept_online_order: online orders sit at status='pending' until accepted,
-- so acceptance (not submission) is the right moment to also auto-set
-- ready_at when the owner has turned this on for the order's channel.
-- delivered_at is deliberately left untouched — an online delivery order
-- still gets the real two-stage ready → delivered handoff, this toggle only
-- ever skips the "ready" click.
