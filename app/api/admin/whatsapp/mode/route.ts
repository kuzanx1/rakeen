...
export async function POST(request: NextRequest) {
  ...
  const { conversationId, mode } = await request.json().catch(() => ({} as { conversationId?: number; mode?: string }));
  if (!conversationId || (mode !== "ai" && mode !== "human")) {
  ...
  const { error } = await admin.from("rakeen_support_conversations").update({ mode, taken_over_by: null }).eq("id", conversationId);
